class Item < ActiveRecord::Base
  ...
  def self.all_paid_items
    items = Item.joins(:invoice).where(invoices: {status: 'PAID'}).sum(:total)
  end

  def self.paid_items_grouped_by_week(start)
    items = Item.joins(:invoice).where(invoices: {status: 'PAID', date_of_an_invoice: start.beginning_of_day..Time.zone.now})
    items.group_by { |i| i.invoice.date_of_an_invoice.beginning_of_week }
  end

  def self.pending_items_grouped_by_week(start)
    items = Item.joins(:invoice).where(invoices: {status: 'PENDING', date_of_an_invoice: start.beginning_of_day..Time.zone.now})
    items.group_by { |i| i.invoice.date_of_an_invoice.beginning_of_week }
  end
